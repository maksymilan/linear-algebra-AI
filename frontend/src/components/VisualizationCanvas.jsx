...
const MatrixInput = ({ matrix, onMatrixChange, dimension }) => {
    ...
    const handleSliderChange = (e, r, c) => {
        const numValue = parseFloat(e.target.value);
        const newMatrix = matrix.map(row => [...row]);
        newMatrix[r][c] = isNaN(numValue) ? 0 : numValue;
        onMatrixChange(newMatrix);
    };

    return (
        <div className="flex flex-col gap-4 w-full">
            <div className="grid gap-x-2 gap-y-3" style={{ gridTemplateColumns: `24px repeat(${dimension}, 1fr)`}}>
                <div></div>
                {colLabels.map(label => <div key={`col-${label}`} className="text-center font-medium text-[#868E96]">{label}</div>)}
                {rowLabels.map((rowLabel, r) => (
                    <React.Fragment key={`row-${rowLabel}`}>
                        <div className="flex items-center justify-center font-medium text-[#868E96]">{rowLabel}</div>
                        {colLabels.map((_, c) => (
                            <div key={`cell-${r}-${c}`} className="flex flex-col gap-1">
                                <input
                                    ...
                                    className="w-full text-center py-1.5 border border-[#DEE2E6] rounded text-[#212529] focus:border-black focus:ring-1 focus:ring-black outline-none transition-colors"
                                />
                                <input 
                                    type="range" 
                                    min="-5" 
                                    ...
                                    onChange={(e) => handleSliderChange(e, r, c)}
                                    className="w-full h-1 bg-[#DEE2E6] rounded-lg appearance-none cursor-pointer accent-black"
                                    style={{
                                        accentColor: '#000000'
                                    }}
                                />
